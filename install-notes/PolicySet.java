...
public class PolicySet
{
    ...
    {
	System.out.println ("Joe Russack Modified 4/25");
        if (argv.length < 6)
        {
            System.out
	...
	if(filter != null) {
        ...
        if(bundleFilter != null) {
                System.out.println("Got bundleFilter: " + bundleFilter);
        }
        if (containerType == Constants.COLLECTION)
        {
	    System.out.println ("Collections is set..");
                ...
                while (i.hasNext())
                {
                    ...
                    for (Bundle bundle : bundles) {
                        ...
                        for (Bitstream bitstream : bitstreams) {
                            if ( (filter == null || bitstream.getName().indexOf(filter) != -1) &&
                                 (bundleFilter == null || isInBundle(bitstream,bundleFilter))) {
                                // is this a replace? delete policies first
                                if (isReplace || clearOnly) {
                                    authorizeService.removeAllPolicies(c, bitstream);
	                            System.out.println("Clearing bitstream policy on item: "+myitem.getID()+" steam name: "+bitstream.getName());
                                }
                                if (!clearOnly) {
                                    // before create a new policy check if an identical policy is already in place
                                    if (!authorizeService.isAnIdenticalPolicyAlreadyInPlace(c, bitstream, group, actionID, -1)) {
					System.out.println("Adding bitstream policy on item: '"+myitem.getID()+"' stream name: '"+bitstream.getName());
                                        // now add the policy
                                        ResourcePolicy rp = resourcePolicyService.create(c);
